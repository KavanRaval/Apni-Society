  ...
  Edit modal
    ...
    <div class="modal-dialog">
        ...
        <div class="modal-header">
            <h5 class="modal-title" id="exampleModalLabel">Edit SocietyMember</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        <div class="modal-body">
        <form action="societymemberchairman.php" method="POST">
        <input type="hidden" name="societyedit" id="societyedit">
            <div class="mb-3">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" name="passwordedit" id="passwordedit" required>
            </div>
    ...
    <div class="modal-dialog">
        ...
        <div class="modal-body px-1">
            <form action="societymemberchairman.php" method="POST">
                <input type="hidden" name="societyiddelete" id="societyiddelete">
    ...
    <div class="container-fluid">
        ...
        <ul class="navbar-nav me-auto mb-2 mb-lg-0">
            ...
            <li class="nav-item">
            <a class="nav-link" href="societymemberchairman.php">Society Info</a>
            </li>
            <li class="nav-item">
    ...
    <?php 
        if($_SERVER['REQUEST_METHOD']=='POST'){
            ...
            if(isset($_POST['societyiddelete'])){
                $sid = $_POST['societyiddelete'];
                $sql = "DELETE FROM `societymember` WHERE `societymember`.`SocietyMemberID` = $sid";
           ...
           elseif(isset($_POST['societyedit'])){
                $sid = $_POST['societyedit'];
                ...
                $sql = "UPDATE `societymember` SET `FirstName` = '$fname', `LastName` = '$lname', `DateOfBirth` = '$date', `Email` = '$email', `PhoneNo` = $moblieno, `Password` = '$password' WHERE `SocietyMemberID` = $sid";
                $result = mysqli_query($conn,$sql);
                if($result){
                    echo "<div class='alert alert-success alert-dismissible fade show' role='alert'>
                    <strong>Success!</strong> Edited!
            ...
            $sql = "INSERT INTO `societymember` (`SocietyMemberID`, `FirstName`, `LastName`, `DateOfBirth`, `Email`, `PhoneNo`, `Password`) VALUES (NULL, '$fname', '$lname', '$date', '$email', $moblieno, '$password');";
            $result = mysqli_query($conn,$sql);
            if($result){
                echo "<div class='alert alert-success alert-dismissible fade show' role='alert'>
                <strong>Success!</strong> SocietyMember Added!
                <button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button>
              </div>";
            }
            else{
    ...
    <div class="container my-3 p-5 border border-3 rounded shadow-lg border-dark bg-dark text-white">
        <h2 class="mb-4">Add SocietyMember</h2>
        <form action="societymemberchairman.php" method="POST">
            <div class="mb-3">
                <label for="firstname" class="form-label">First Name</label>
        ...
        <tbody>
            <?php
                ...
                while($row = mysqli_fetch_assoc($result)){
                    ...
                    <td>".$row['PhoneNo']."</td>
                    <td>".$row['Password']."</td>
                    <td><button class='edit btn' id=".$row['SocietyMemberID']." >Edit</button> <button class='delete btn' id=d".$row['SocietyMemberID']." >Delete</button></td>
                    </tr>";
                    $i = $i+1;
                }
            ?>
